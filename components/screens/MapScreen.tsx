...
const R_map_to_global = math.matrix([
...
function to_map_coords(latitude: number, longitude: number) {
  ...
  // @ts-ignore
  const p_px_map = math.matrix([
    [p_map.get([0, 0]) * (w_px / w), p_map.get([0, 1]) * (w_px / w)],
  ]);
...
export const MapScreen = function ({ navigation }: any): JSX.Element {
  ...
  const { x, y } = location
    ? to_map_coords(location.coords.latitude, location.coords.longitude)
    : { x: 0, y: 0 };
